...
RUN cd /usr/local/src/nginx-1.10.1 \
	...
	&& ./configure --prefix=/opt/nginx --with-ld-opt="-Wl,-rpath,/usr/local/luajit/lib" --add-module=../ngx_devel_kit-0.3.0 --add-module=../lua-nginx-module-0.10.6 --add-module=../ngx_http_redis-0.3.7 \
	&& make -j2 \
	&& make install
